...
async function label_send_map_load()
{
    ...
    UI.label_send_map_cont.insertAdjacentHTML("beforeend", "<strong>시리얼로 보낼 문자(한 글자) 정의</strong>");
    ...
    for (let i = 0; i < label_count; i += 1)
    {
        ...
        const child = `<div><strong>${label_i}</strong><input type="text" id="val_${i}" placeholder="${base_val}" maxlength="1"></input></div>`;
